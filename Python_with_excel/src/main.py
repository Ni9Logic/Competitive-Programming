...
def from_pdf_excel():
    ...
    for tables in range(1, len(converted.sheetnames) + 1):
        ...
        if invoice_sheet_inc + 3 > len(converted.sheetnames) + 1:
            ...
            if '\n' in str(invoice['F1'].value) and not invoice['A1'].value == "Shop Information":
                ...
                print(invoice_sheet_inc, invoice_number)
    ...
    #     if j == len(converted.sheetnames) + 1:
    #         continue
...
def main():
    ...
    print(f"Program Executed in {execution_time} secs...", end='')
...
def from_pdf_excel():
    ...
    for tables in range(1, len(converted.sheetnames) + 1):
        ...
        if invoice_sheet_inc + 3 > len(converted.sheetnames) + 1:
            ...
            if invoice['B1'].value == None and invoice['A1'].value == "Shop Information":
                ...
                if '\n' in strings:
                    strings = "".join(
                        strings[strings.index('-') + 1:strings.index('\n')])
                else:
                    strings = "".join(strings[strings.index('-') + 1::])
            ...
            if '\n' in str(invoice['F1'].value) and not invoice['A1'].value == "Shop Information":
                ...
                invoice_number = str(invoice['F1'].value)
                invoice_number = list(
                    invoice_number[invoice_number.index('-') + 1:])
                invoice_number = "".join(invoice_number)
    ...
    for sale in range(1, len(converted.sheetnames) + 1):
        if invoice_sales_inc + 3 > len(converted.sheetnames) + 3:
            break
        temp = converted[f'Table {invoice_sales_inc}']
        if temp['A1'].value == "Shop Information" and temp['B1'].value == "Distributor Information" or temp['A1'].value == "CASH MEMO / SALE INVOICE":
            invoice_sales_inc += 1
            continue
        while invoice_sales_inc + 3 <= len(converted.sheetnames) + 1:
    ...
    for j in range(1, len(converted.sheetnames) + 1):
        if j == len(converted.sheetnames) + 1:
            continue
        temp = converted[f'Table {invoice_quantity_inc}']
        if temp['A1'].value == "Shop Information" and temp['B1'].value == "Distributor Information" or temp['A1'].value == "CASH MEMO / SALE INVOICE":
            invoice_sheet_inc += 1
            continue
        while invoice_quantity_inc + 3 <= len(converted.sheetnames) + 3:
...
main()
>>>>>>> cf9e0e85e9d5c871a4b2bff2dfa4cd973e504206
